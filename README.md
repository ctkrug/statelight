...
**▶ Live demo: [apps.charliekrug.com/statelight](https://apps.charliekrug.com/statelight/)**
...
`<script>` tag next to any plain-object state machine (no framework, no build
step, no library lock-in) and watch its current state and transition path
light up live as your app runs.

## Who it's for

JavaScript developers who hand-roll small state machines: a `state` field and a
chunk of `if`/`switch` logic, instead of reaching for XState or a full FSM
library. That's fine until you're three transitions deep in a bug with no
visibility into what state you're actually in or how you got there. Existing FSM
visualizers assume you already bought into their library and their build
...
- Watches a plain JavaScript object's state property (via a transparent
  getter/setter, so your code doesn't change) and renders a floating panel
...
- [x] Panel ergonomics: collapse/expand, drag to reposition, persisted via
      `localStorage`.
- [x] Multi-machine support: `attach()` more than one state machine on the
      same page; panels cascade by default and detach independently.
...
No React, no Vue, no npm install required. `attach()` works on any plain object
with a mutable state property.

## Demo site

[`site/`](site/) is a self-contained landing page with a real, running
Statelight instance as its hero, not a screenshot. Serve the folder with any
static file server and open it:
...
(`file://` won't work: the page uses ES module imports.)

## Stack

Vanilla JavaScript (ES modules), zero runtime dependencies. `esbuild` bundles
the library for distribution; `node:test` plus `jsdom` cover the unit tests.
...
npm run build # bundle src/ into dist/ (and site/)
```
